...
```sh
go get github.com/voltariafinance/go-sdk/v2
```

Construct a client with your API key and make your first read call:
...
func main() {
	// Production. For sandbox, use your "sandbox_" key instead:
	// c, err := client.NewVoltaria("sandbox_...")
	c, err := client.NewVoltaria("live_...")
	if err != nil {
		log.Fatalf("failed to create client: %v", err)
	}

	loans, err := c.Loans.ListLoans(context.Background(), &api.ListLoansRequest{})
	if err != nil {
		log.Fatalf("failed to list loans: %v", err)
	}
...
The base URL is derived automatically from your API key prefix:
...
Passing an explicit base URL (`option.WithBaseURL`) overrides prefix routing.

See [reference.md](./reference.md) for the full API reference.

---

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Voltaria%2FGo)

The Voltaria Go library provides convenient access to the Voltaria APIs from Go.
...
```go
response, err := client.Clients.CreateClient(...)
if err != nil {
...
```go
response, err := client.Clients.WithRawResponse.CreateClient(...)
if err != nil {
    return err
}
...
response, err := client.Clients.CreateClient(ctx, ...)
```
...
response, err := client.Clients.CreateClient(ctx, request, ...)
```
